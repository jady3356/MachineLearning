...
def conv_forward(A_prev, W, b, hparameters):
    ...
    for i in range(m):                               # loop over the batch of training examples
        ...
        #print(str(a_prev_pad.shape)) # (8,8,3)
        for h in range(n_H):                           # loop over vertical axis of the output volume
            for w in range(n_W):                       # loop over horizontal axis of the output volume
                for c in range(n_C):                   # loop over channels (= #filters) of the output volume
                    
                    # Find the corners of the current "slice" (≈4 lines)
                    vert_start = h * stride # 这里的stride很容易忘记。。。
                    ...
                    #print(str(a_slice_prev.shape)) # (2,2,3)
        ...
        for h in range(n_H):                     # loop on the vertical axis of the output volume
            for w in range(n_W):                 # loop on the horizontal axis of the output volume
                for c in range (n_C):            # loop over the channels of the output volume
                    ...
                    #print(str(a_prev_slice)) #(3,3)
                    
                    # Compute the pooling operation on the slice. Use an if statment to differentiate the modes. Use np.max/np.mean.
                    if mode == "max":
                        A[i, h, w, c] = np.max(A_prev[i,vert_start:vert_end,horiz_start:horiz_end,c]) # 为什么不能直接用np.max(a_prev_slice)
                        #print(str(a_prev_slice))
                        #print(str(A[i, h, w, c]))
                    elif mode == "average":
