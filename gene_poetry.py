...
def neural_network(model='lstm', rnn_size=128, num_layers=2):  
    ...
    def cell():
	lstm = cell_fun(rnn_size, forget_bias=0.0, state_is_tuple=True,reuse=tf.get_variable_scope().reuse)
	return lstm  
    #cell = cell_fun(rnn_size, forget_bias=0.0, state_is_tuple=True,reuse=tf.get_variable_scope().reuse)  
    cell = rnn.MultiRNNCell([cell() for _ in range(num_layers)])  
...
#print(gen_poetry()) 
save_to_file('poetryResult.txt', gen_poetry())
